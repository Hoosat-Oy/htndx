...
class HTNProcessManager extends EventEmitter{
	...
	getDeployableBinary(...args) {
		...
		switch(buildType) {
			...
			case 'local': {
				return path.join(os.homedir(),'.htndx','bin',utils.platform, ...args)+this.PLATFORM_BINARY_EXTENSION;
			}			
		}
	}
	...
	enumerateApps() {
		...
		return packages.map(({ folder, pkg }) => {
			let { name, description, main, engines, htndx } = pkg;
			return {
				folder, name, description, main, engines, ...htndx
			}
		});
	}
...
module.exports = HTNProcessManager;
